...
정적 웹사이트가 즉시 사용할 수 있도록 public/data/ 및 루트 data/ 디렉토리에 JSON 파일로 저장합니다.
"""
...
def save_json_dual(rel_path: str, data: dict):
    """두 디렉토리에 동일하게 JSON을 저장합니다."""
    for d in TARGET_DATA_DIRS:
        try:
            ...
            logging.warning(f"JSON 저장 실패 ({full_path}): {e}")
...
def run_pipeline():
    ...
    save_json_dual("stocks.json", stocks_meta)
    ...
    for stock in STOCKS_CONFIG:
        ...
        try:
            ...
            save_json_dual(os.path.join("predictions", f"{code}.json"), detail_data)
            ...
            logging.error(f"❌ [{name}] 분석 중 오류 발생: {stock_err}")
    ...
    save_json_dual("latest_summary.json", summary_data)
